...
rhit.FB_KEY_CHARACTER = "character";
...
rhit.ListPageController = class {
	constructor() {
		...
		document.querySelector("#submitAddQuote").addEventListener("click", (event) => {
			...
			const character = document.querySelector("#inputCharacter").value;
			rhit.fbMovieQuotesManager.add(quote, movie, character);
		});
		...
		$("#addQuoteDialog").on("show.bs.modal", (event) => {
			...
			document.querySelector("#inputCharacter").value = "";			
		});
	...
	_createCard(movieQuote) {
		if(!movieQuote.character) {
			movieQuote.character = "";
		}
		return htmlToElement(`<div class="card">
		<div class="card-body">
			<h5 class="card-title">${movieQuote.quote}</h5>
			<h6 class="card-subtitle mb-2 text-muted">${movieQuote.movie}</h6>
			<h6 class="card-subtitle mb-2 text-muted">${movieQuote.character}</h6>
		</div>
	</div>`);
	}
...
rhit.MovieQuote = class {
	constructor(id, quote, movie, character) {
		this.id = id;
		this.quote = quote;
		this.movie = movie;
...
rhit.FbMovieQuotesManager = class {
	...
	getMovieQuoteAtIndex(index) {
		...
		const mq = new rhit.MovieQuote(docSnapshot.id,
			...
			docSnapshot.get(rhit.FB_KEY_CHARACTER));
		return mq;
	}
}
...
rhit.DetailPageController = class {
	constructor() {
		...
		document.querySelector("#submitEditQuote").addEventListener("click", (event) => {
			...
			const movie = document.querySelector("#inputMovie").value;
			const character = document.querySelector("#inputCharacter").value;
			rhit.fbSingleQuoteManager.update(quote, movie, character);
		});
		...
		$("#editQuoteDialog").on("show.bs.modal", (event) => {
			...
			if(rhit.fbSingleQuoteManager.character) {
				document.querySelector("#inputCharacter").value = rhit.fbSingleQuoteManager.character;
			}
		});
	...
	updateView() {
		...
		if(rhit.fbSingleQuoteManager.character){
			document.querySelector("#cardCharacter").innerHTML = rhit.fbSingleQuoteManager.character;
		}
	}
}
...
rhit.FbSingleQuoteManager = class {
	...
	get character() {
		return this._documentSnapshot.get(rhit.FB_KEY_CHARACTER);
	}
}
